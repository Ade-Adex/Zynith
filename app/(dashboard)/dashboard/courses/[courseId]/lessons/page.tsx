...
//     const completedIds = new Set(dbEnrollment.completedLessons)
//     return activeModule.lessons.every((lesson) => completedIds.has(lesson.id))
//   }, [activeModule, dbEnrollment?.completedLessons])
...
//     // Critical state resets across layout transitions
//     setVideoEnded(false)
//     setTextRead(false)
//     setSelectedAnswers({})
...
//                       }
//                     }}
//                     className="w-full sm:w-auto px-8 py-2.5 text-sm! mt-6 rounded-2xl bg-neutral-900 text-white dark:bg-white dark:text-black font-black transition-all hover:bg-neutral-800 dark:hover:bg-neutral-100 active:scale-[0.98] cursor-pointer"
//                   >
//                     {isQuizSubmitted && !quizResult?.passed
//                       ? 'Try Again'
//                       : 'Submit Quiz'}
...
//                       <input
//                         key={activeModule.id} // <-- Adding a key handles auto-resetting the input element when switching modules natively
//                         type="url"
//                         placeholder="https://github.com/..."
//                         // If submitted, show the locked database URL, otherwise read from your local typing buffer
//                         value={
//                           assignmentSubmitted && assignmentSubmissionInfo
//                             ? assignmentSubmissionInfo.submissionUrl
//                             : assignmentInputBuffer
//                         }
//                         onChange={(e) =>
...
//                           !isModuleLessonsComplete
//                         }
//                         className="w-full sm:w-auto px-8 py-2.5 rounded-2xl text-sm! bg-amber-500 hover:bg-amber-400 text-black font-black transition-all active:scale-[0.98] disabled:opacity-40 disabled:pointer-events-none cursor-pointer"
//                       >
//                         {isAssignmentSubmitting
...
export default function CourseWorkspacePage() {
  ...
  // --- CRITICAL SUBMISSION REQUIRMENTS ENGINE ---
  const requirementsMet = useMemo(() => {
    // 1. Content Consumption check
    const isVideoDone =
      activeLesson?.contentType === 'video' ? videoEnded : true
    const isTextDone = activeLesson?.contentType === 'text' ? textRead : true

    // 2. Quiz completion check (Checks lesson quiz or module quiz depending on availability)
    const activeQuiz = activeLesson?.quiz || activeModule?.quiz
    const isQuizPassed = activeQuiz ? (quizResult?.passed ?? false) : true

    // 3. Assignment submission requirement per module checklist
  ...
  const handleMarkComplete = async () => {
    ...
    startTransition(async () => {
      // Create final payload. If this is the last lesson element of the module, pack the module completion alongside it.
      const payload: UpdatePayload = {
        newCompletedLessonId: activeLesson.id,
      }

      if (activeModule && isModuleLessonsComplete) {
        payload.newCompletedModuleId = activeModule.id
      }

      const result = await updateEnrollmentProgressAction(
        user._id,
        courseId,
        payload,
      )
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6">
                  <div className="text-center sm:text-left">
                    ...
                    <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">
                      {currentLessonIsCompleted
                        ? '✨ Completed & verified.'
                        : 'Finish video, text content, active quizzes and assignments to clear.'}
                    </p>
                  </div>
                  <button
                    onClick={handleMarkComplete}
